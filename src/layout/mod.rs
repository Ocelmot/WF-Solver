...
pub trait Layout<V: CellValue>: Clone {
    ...
    fn add_cell_possibilities(
        &mut self,
        coord: &Self::Coordinate,
        possibilities: &HashMap<V, usize>,
    ) {
        if let Some(cell) = self.get_cell_mut(coord) {
    ...
    fn add_cells_possibility_count(
        &mut self,
        coords: Vec<Self::Coordinate>,
        possibility: &V,
        count: usize,
    ) {
        for coord in coords {
    ...
    fn remove_cell_possibility_count(
        &mut self,
        coord: &Self::Coordinate,
        possibility: &V,
        count: usize,
    ...
    fn remove_cells_possibility_count(
        &mut self,
        coords: Vec<Self::Coordinate>,
        possibility: &V,
        count: usize,
    ...
    /// Sets a cell to an uncollapsed state with no weights.
    fn clear_cell(&mut self, coord: &Self::Coordinate) {
        if let Some(cell) = self.get_cell_mut(coord) {
            cell.set_possibilities(HashMap::new());
        }
    }

    /// Merges the given weights into those of the cell at the coordinate.
    /// 
    /// More information about how the merge occurs can be found in the
    /// documentation for the cell function [Cell::merge_cell_possibilities()].
    fn merge_cell_possibilities(
        &mut self,
        coord: &Self::Coordinate,
        op: Operation,
        func: Function,
        ...
        if let Some(cell) = self.get_cell_mut(coord) {
            cell.merge_cell_possibilities(op, func, weights);
        }
    }
    ...
    fn candidates<'a>(&'a mut self) -> impl 'a + Iterator<Item = (Self::Coordinate, &mut Cell<V>)>
    where
        V: 'a,
    {
